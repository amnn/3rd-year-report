...
## Implications of a Human Oracle

We must carefully consider the interface we provide to the user so that they may
act as oracle. This will direct our implementations for $\textsc{Counter}^*$ and
$\textsc{Member}^*$.
...
requires some careful thought, which we will delve into in
Section\ \ref{sec:counter-samples}.
...
asking the same question multiple times. We will compare some minimisation
techniques in Section\ \ref{sec:membership}.
