...
an interface to the user to act as the oracle, in the form of implementations of
the \texttt{member*} and \texttt{counter*} functions.

In the case of the former, it is reasonable (and necessary) to pose the query
directly to the user (Figure\ \ref{list:int-member}). A user who knows the
...
(by convention), the semantics of other non-terminals is available only to the
...
requires an unreasonable expectation upon the user, however. To do so, they
would have to analyse the grammar themselves, locate an erroneous rule, and
construct a counter-example highlighting this fact to the algorithm in such a
way that the algorithm removes the offending rule. Even if we prune the grammar
of redundant rules (as we do), this is a time consuming task.

To mitigate some of the complexity of this task, we hide the grammar from the
user, and instead rely upon:
...
terminate and return the wrong grammar. We can improve the situation with larger
sample sizes, but this is a cost we shoulder gladly for the weight it takes off
the user.
